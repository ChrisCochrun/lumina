...
use cosmic::iced::core::{
    Background, Border, Clipboard, Length, Padding, Point, Rectangle, Renderer as _,
    Shadow, Shell, Size, Vector, Widget, overlay, renderer, window,
};
use cosmic::iced::{Alignment, Pixels, alignment};
use cosmic::{Element, Renderer};
use std::collections::{HashMap, HashSet};
use std::hash::Hash;
use std::time::{Duration, Instant};

const DEFAULT_ANIMATION_DURATION: Duration = Duration::from_millis(120);
const DEFAULT_DRAG_LIFT: f32 = 10.0;
const DEFAULT_DRAG_THRESHOLD: f32 = 6.0;
const SHADOW_BLUR_RADIUS: f32 = 16.0;
...
impl SlotAnimation {
    ...
    fn current_position(&self, duration: Duration) -> Point {
        ...
        let progress =
            (started_at.elapsed().as_secs_f32() / duration_secs).clamp(0.0, 1.0);
    ...
    Key: Clone + Eq + Hash + 'static,
{
    ...
    fn sync_slot_positions(&self, state: &mut State<Key>, slots: &[LocalSlot<Key>]) {
        ...
        let reordered_keys = reordered_keys_for_drag(
            &ordered_keys,
            &locked_by_key,
            &drag_key,
            target_index,
...
impl<'a, Key, Message> Widget<Message, cosmic::Theme, Renderer>
    ...
    Message: 'a,
{
        ...
        match event {
            Event::Mouse(mouse::Event::ButtonPressed(mouse::Button::Left)) => {
                if state.drag.is_none()
                    ...
                    && let Some((index, child_layout)) =
                        child_layouts.iter().enumerate().find(|(_, child_layout)| {
                            child_layout.bounds().contains(cursor_pos)
                        })
                    && !self.locked.get(index).copied().unwrap_or(false)
                    && let Some(reorder_index) =
                        reorderable_index_for_child(&self.locked, index)
                {
                    let child_bounds = child_layout.bounds();
            ...
            Event::Mouse(mouse::Event::CursorMoved { .. }) => {
                    ...
                    && let Some(cursor_pos) = cursor.position()
                {
                    ...
                    if distance >= DEFAULT_DRAG_THRESHOLD {
                        ...
                        let slots =
                            self.item_local_slots_from_layout(bounds, &child_layouts);
                        self.sync_slot_positions(state, &slots);
                        shell.capture_event();
                        shell.request_redraw();
                        return;
    ...
    fn draw(
            ...
            .zip(layout.children())
        {
            ...
            let child_bounds = child_layout.bounds();
            let base_local =
                Point::new(child_bounds.x - bounds.x, child_bounds.y - bounds.y);
            let target_local =
        ...
        if let Some((_key, child, child_tree, child_layout, drag)) = drag_item {
            ...
            let translation =
                Vector::new(drag_local.x - base_local.x, drag_local.y - base_local.y);
...
fn draw_drag_backdrop(renderer: &mut Renderer, theme: &cosmic::Theme, bounds: Rectangle) {
