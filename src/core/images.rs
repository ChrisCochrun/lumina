    ...
    pub fn sort(&mut self) {
        match self.sorting_method {
            Sort::AccessTime(SortDirection::Descending) => {
                self.items
                    .sort_by_key(|image| std::cmp::Reverse(image.accessed_at));
            }
            Sort::AccessTime(SortDirection::Ascending) => {
                self.items.sort_by_key(|image| image.accessed_at);
            }
            ...
            Sort::CreatedTime(SortDirection::Descending) => {
                self.items
                    .sort_by_key(|image| std::cmp::Reverse(image.created_at));
            }
            Sort::CreatedTime(SortDirection::Ascending) => {
                self.items.sort_by_key(|image| image.created_at);
            }
