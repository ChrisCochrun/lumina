    ...
    pub fn sort(&mut self) {
        match self.sorting_method {
            Sort::AccessTime(SortDirection::Descending) => {
                self.items.sort_by_key(|presentation| {
                    std::cmp::Reverse(presentation.accessed_at)
                });
            }
            Sort::AccessTime(SortDirection::Ascending) => {
                self.items
                    .sort_by_key(|presentation| presentation.accessed_at);
            }
            ...
            Sort::CreatedTime(SortDirection::Descending) => {
                self.items.sort_by_key(|presentation| {
                    std::cmp::Reverse(presentation.created_at)
                });
            }
            Sort::CreatedTime(SortDirection::Ascending) => {
                self.items
                    .sort_by_key(|presentation| presentation.created_at);
            }
