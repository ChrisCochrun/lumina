...
* TODO [#A] Fix actions not staying with a slide when user adds more items into service
Because actions are just mapped into a slide by a hashmap instead of the slide having the action in it's struct, we are going to have to do some reconstruction of the hashmap when items are added. Maybe, it's best to keep a linear copy of the slide indexes or even create a unique slide id for each slide and map those to the actions so they can be looked up later.

I think this might be getting fixed, but actions are not saved with the file, so that would be a another thing to consider adding.
...
* TODO [#A] Fix song imports so that they actually get rid of extra cruft
...
* TODO [#A] Fix loading file with pdf not showing the pdf properly
I think this has something to do with the pdf rendering not starting up yet when loading from a file? Like maybe a hook i missed?

Maybe this isn't even a real blocker for using this at church. Just need it to build.

I have found a fix that will work for now, but it causes loading to take a long time when there are pdf files in there.

* TODO [#A] Find a way to check if an item is in the library on load so that we can import it into the library

This could play into font systems...
If an item needs to be imported and relies on a font that the current machine does not have, we could ask the user if they'd like to move that file from the cache dir to the system's font directory and therefore install it?

* TODO [#A] Good keyboard shortcuts for the song editor so that making songs is faster and more intuitive
...
* TODO [#B] Add an access time to the database so that we can sort library items by last used or edited.
* TODO [#B] Make audio is song editor able to increase speed so the user can create the song a little faster if they desire.
...
* TODO Make better tooltips for items in libraries and in the service list.

Some ideas for this, include a basic tooltip to show the full title of the item and maybe a little extra metadata. But after a certain time out period, maybe we can show a thumbnail version of some items, like thumbnails of the videos and thumbnails of the images, so you can preview it without opening it.

* DONE [#A] Add a migration/manifest of file format for saving and loading
CLOSED: [2026-09-21 Mon 13:18]
In order to be able to update the structure of the types I will need to add a manifest or something to track the version of save files and then allow them to be updated to the new types on load. This should be done before V1 so that new versions of Lumina do not fail to load save files as the app grows.

Instead of adding a file that describes the format we are saving in, since we are using rust and ron, we can just wrap the serviceitem and slide types in versioned enums that mimic the history of types. V1 will be what we have now, but over time, we can change the layout of the structs and each enum variant can just track the next version, then to serialize and deserialize the main type, we simply use the enum as the actual serializer and deserializer and just convert before or after the serialization. This means we only need to add a new version on each change and rust's compiler will ensure we are adding each conversion the way that we should.

* DONE [#A] Make loading not block the UI
CLOSED: [2026-06-23 Tue 10:19]
Not working just yet but loading is faster.

* DONE [#A] Loading and saving need to have a progress indicator of some sort
CLOSED: [2026-06-23 Tue 10:19]
...
* DONE [#A] Custom widget for slides to work with animations
CLOSED: [2026-07-01 Wed 10:51]
...
* DONE [#B] Build an Animation type that will hold all the info for what a slide animation is.
CLOSED: [2026-06-26 Fri 17:08]
...
* DONE Song editor audio slider not working
CLOSED: [2026-06-18 Thu 13:18]
* DONE Preview mode needs to allow for a larger preview of the slide if the library is closed
CLOSED: [2026-05-30 Sat 15:15]
* DONE Fix the scrolling when switching slides for preview and grid
CLOSED: [2026-06-19 Fri 05:43]
